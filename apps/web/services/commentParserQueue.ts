...
export const commentParseQueue = new Queue<
  ...
  boolean,
  string
>('COMMENT-PARSE-QUEUE', { connection: redisClient });

// Initialize worker only when needed to avoid connection issues
let commentParseWorker: Worker<CommentParseJobData, boolean, string> | null =
  null;

const getCommentParseWorker = () => {
  if (!commentParseWorker) {
    commentParseWorker = new Worker(
      'COMMENT-PARSE-QUEUE',
