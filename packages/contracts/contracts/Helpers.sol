...
library Helpers {
    ...
    function chainToAxelarName(Chain c) internal pure returns (string memory) {
        if (c == Chain.ETHEREUM_SEPOLIA) {
            return "Ethereum"; // or "Ethereum" / "Sepolia" depending on Axelar's expectation
        } else if (c == Chain.BASE_SEPOLIA) {
            return "Base"; // adjust if Axelar expects "Base" or "Base Sepolia" etc.
        } else {
    ...
    function bytesToStringAddress(bytes memory b) internal pure returns (string memory) {
        if (b.length == 20) {
    ...
    function addressToHexString(address _addr) internal pure returns (string memory) {
        // Using OpenZeppelin's Strings.toHexString(uint256, length)
